...
export class DiscordService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.client.on('messageCreate', async (message) => {
      ...
      if (message.content === '!unsubscribe') {
        await this.unsubscribe(message.author.id);
  ...
  async unsubscribe(userId: string): Promise<Subscriber> {
  ...
  async sendDirectMessage(messageDto: MessageDto): Promise<void> {
    ...
    for (const subscriber of activeSubscribers) {
      try {
        ...
        console.error(`Не удалось отправить сообщение пользователю ${subscriber.username}:`, error);
      }
    }
  }
}