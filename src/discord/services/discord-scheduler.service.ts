...
export class DiscordSchedulerService {
  ...
  scheduleMessageToRole(
    ...
    this.logger.log(
      `Запланирована отправка сообщения для роли "${roleName}" в ${scheduledTime.toLocaleString()}`,
    );

    return scheduleId;
  }

  scheduleMessageToAllMilitia(
    content: string,
    scheduledTime: Date,
    messageId: string,
  ): string {
    ...
    this.logger.log(
      `Запланирована отправка сообщения всем ополченцам в ${scheduledTime.toLocaleString()}`,
    );
  ...
  cancelScheduledMessage(scheduleId: string): boolean {
    ...
    if (timeout) {
      ...
      this.scheduledMessages.delete(scheduleId);
      this.logger.log(
        `Отменена запланированная отправка сообщения с ID ${scheduleId}`,
      );
      return true;
    }
    return false;
